...
class GWR_Documents_Admin {
	...
	private static function attachments_panel( $booking ) {
		...
		echo '<div class="gwr-table-wrap"><table class="widefat"><thead><tr><th>' . esc_html__( 'File', 'gest-web-rent' ) . '</th><th>' . esc_html__( 'Tipo', 'gest-web-rent' ) . '</th><th>' . esc_html__( 'Stato verifica', 'gest-web-rent' ) . '</th><th>' . esc_html__( 'Visibilita', 'gest-web-rent' ) . '</th><th>' . esc_html__( 'Caricato', 'gest-web-rent' ) . '</th><th>' . esc_html__( 'Azioni', 'gest-web-rent' ) . '</th></tr></thead><tbody>';
		if ( empty( $attachments ) ) {
			echo '<tr><td colspan="6">' . esc_html__( 'Nessun allegato caricato.', 'gest-web-rent' ) . '</td></tr>';
		}
		foreach ( $attachments as $attachment ) {
			echo '<tr><td><strong>' . esc_html( $attachment['description'] ?: basename( (string) $attachment['file_path'] ) ) . '</strong><small>' . esc_html( 'customer' === ( $attachment['source'] ?? '' ) ? __( 'Caricato dal cliente', 'gest-web-rent' ) : __( 'Caricato da admin', 'gest-web-rent' ) ) . '</small></td><td>' . esc_html( $attachment['mime_type'] ) . '</td><td>' . esc_html( $attachment['verification_status'] ?? 'pending' ) . '</td><td>' . esc_html( $attachment['visibility'] ) . '</td><td>' . esc_html( GWR_Bookings::format_datetime( $attachment['created_at'] ) ) . '</td><td><div class="gwr-row-actions"><a href="' . esc_url( GWR_Attachment_Service::download_url( $attachment ) ) . '">' . esc_html__( 'Download', 'gest-web-rent' ) . '</a>';
			echo '<a href="' . esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=gwr_booking_attachment_verify&attachment_id=' . absint( $attachment['id'] ) . '&verification_status=approved' ), 'gwr_booking_attachment_verify_' . absint( $attachment['id'] ) ) ) . '">' . esc_html__( 'Approva', 'gest-web-rent' ) . '</a><a href="' . esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=gwr_booking_attachment_verify&attachment_id=' . absint( $attachment['id'] ) . '&verification_status=rejected' ), 'gwr_booking_attachment_verify_' . absint( $attachment['id'] ) ) ) . '">' . esc_html__( 'Rifiuta', 'gest-web-rent' ) . '</a>';
